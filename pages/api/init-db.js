...
// GitHub: pages/api/init-db.js 에 덮어씌우세요
...
export default async function handler(req, res) {
  ...
  try {
    ...
    await sql`CREATE TABLE IF NOT EXISTS weekly_summary (
    ...
    await sql`CREATE TABLE IF NOT EXISTS daily_completed (
      id SERIAL PRIMARY KEY,
    ...
    await sql`CREATE TABLE IF NOT EXISTS company_stats (
      id SERIAL PRIMARY KEY,
      week_label VARCHAR(10) NOT NULL REFERENCES weekly_summary(week_label) ON DELETE CASCADE,
    ...
    await sql`CREATE TABLE IF NOT EXISTS elapsed_distribution (
      id SERIAL PRIMARY KEY,
      week_label VARCHAR(10) NOT NULL REFERENCES weekly_summary(week_label) ON DELETE CASCADE,
    ...
    await sql`CREATE TABLE IF NOT EXISTS worker_stats (
      id SERIAL PRIMARY KEY,
      week_label VARCHAR(10) NOT NULL REFERENCES weekly_summary(week_label) ON DELETE CASCADE,
    ...
    await sql`CREATE TABLE IF NOT EXISTS overdue_vehicles (
      id SERIAL PRIMARY KEY,
      week_label VARCHAR(10) NOT NULL REFERENCES weekly_summary(week_label) ON DELETE CASCADE,
