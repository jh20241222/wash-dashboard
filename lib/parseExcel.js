...
export function parseCarwashExcel(buffer, weekLabel) {
  ...
  const over21Simple    = over21.filter(r => r['세차 불가 여부'] === '단순미세차').length;
  const over21Impossible = over21.filter(r =>
    r['세차 불가 여부'] === '세차 불가' || r['세차 불가 여부'] === '세차 불가 스팟'
  ).length;
  ...
  // ── 4. 미조치 차량 목록 (21일↑) ──────────────────────────────
  const overdue = over21.map(r => ({
    plate:    r['차량번호'] || '',
    model:    r['차종명'] || '',
    days:     Number(r['세차경과일']) || 0,
    region:   [r['지역(시/도)'] || '', r['지역(구/군)'] || ''].filter(Boolean).join(' '),
    spot:     r['현재스팟명'] || '',
    company:  r['담당업체'] || '',
    reason:   r['세차 불가 여부'] || '단순미세차',
    carryOver: r['기타'] || '-',
