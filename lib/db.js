async function insertOverdue(db, weekLabel, overdue) {
...
async function insertOneOverdue(db, wl, row) {
  ...
  const db2 = db;
  await db2`INSERT INTO overdue_vehicles(week_label,license_plate,car_model,elapsed_days,region,spot_name,company_name,reason,carry_over)VALUES(${wl},${pl},${mo},${dy},${re},${sp},${co},${rs},${cy})`;
}
...
export async function insertWeekData(weekLabel, data) {
  ...
  // overdue 구조 검증
  console.log('overdue type:', typeof overdue, 'isArray:', Array.isArray(overdue), 'length:', overdue?.length);
  if (overdue.length > 0) {
    console.log('first overdue item:', JSON.stringify(overdue[0]));
    console.log('days type:', typeof overdue[0].days, 'value:', overdue[0].days);
  }
  await db`DELETE FROM overdue_vehicles WHERE week_label=${weekLabel}`;
  await insertOverdue(db, weekLabel, overdue);
}
