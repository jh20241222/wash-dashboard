...
export async function insertWeekData(weekLabel, data) {
  ...
  for (const v of overdue) {
    const pl = String(v.plate||'');
    const mo = String(v.model||'');
    const dy = Math.floor(Number(v.days)||0);
    const re = String(v.region||'');
    const sp = String(v.spot||'');
    const co = String(v.company||'');
    const rs = String(v.reason||'');
    const cy = String(v.carryOver||'-');
    await db`INSERT INTO overdue_vehicles(week_label,license_plate,car_model,elapsed_days,region,spot_name,company_name,reason,carry_over)VALUES(${weekLabel},${pl},${mo},${dy},${re},${sp},${co},${rs},${cy})`;
  }
}
