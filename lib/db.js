...
export async function getWeekData(weekLabel){
  const [summary,daily,companies,elapsed,workers,overdue,partners,models]=await Promise.all([
  ...
  return {summary:summary[0]||null,daily,companies,elapsed,workers,overdue,partners,models};
}
...
export async function insertWeekData(weekLabel,data){
  const{summary:s,daily,companies,elapsed,workers,overdue,completedPlates=[],partners=[],models=[]}=data;
  ...
  if(workers.length>0){
    ...
    // mixed_count = completed_count - round_count 로 별도 UPDATE (컬럼수 제한 우회)
    for(const w of workers) await q('UPDATE worker_stats SET mixed_count=$1 WHERE week_label=$2 AND worker_id=$3',[Math.max(0,safeInt(w.count)-safeInt(w.roundCount)),weekLabel,safeStr(w.id,50)]);
  }
