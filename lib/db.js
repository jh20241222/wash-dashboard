...
export async function insertWeekData(weekLabel,data){
  const{summary:s,daily,companies,elapsed,workers,overdue,completedPlates=[]}=data;
  ...
  if(workers.length>0){
    const wv=workers.map((_,i)=>`($${i*4+1},$${i*4+2},$${i*4+3},$${i*4+4})`).join(',');
    const wp=workers.flatMap(w=>[weekLabel,safeStr(w.id,50),safeInt(w.count),safeFloat(w.avgMinutes)]);
    await q(`INSERT INTO worker_stats(week_label,worker_id,completed_count,avg_work_minutes)VALUES${wv}`,wp);
  }
  ...
  if(overdue.length>0){
    const ov=overdue.map((_,i)=>`($${i*9+1},$${i*9+2},$${i*9+3},$${i*9+4},$${i*9+5},$${i*9+6},$${i*9+7},$${i*9+8},$${i*9+9})`).join(',');
    const op=overdue.flatMap(v=>[weekLabel,safeStr(v.plate,20),safeStr(v.model,100),safeInt(v.days),safeStr(v.region,50),safeStr(v.spot,200),safeStr(v.company,50),safeStr(v.reason,50),safeStr(v.carryOver,30)]);
    await q(`INSERT INTO overdue_vehicles(week_label,license_plate,car_model,elapsed_days,region,spot_name,company_name,reason,carry_over)VALUES${ov}`,op);
  }
}
